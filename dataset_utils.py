...
class CUB_200_2011(object):
    ...
    TRIMAP_FOLDER_NAME = 'segmentations'
    ...
    def __init__(self, base_path=settings.CUB_FOLDER_PATH, images_folder_name=None):
        ...
        self.trimaps_folder = os.path.join(
            self.base_path, self.TRIMAP_FOLDER_NAME)
    ...
    def trimap(self):
        with open(self.images_file, 'r') as images_file:
            for line in images_file:
                parts = line.split()
                assert len(parts) == 2
                folder = self.trimaps_folder
                str2 = parts[1].split('.')
                parts[1] = str2[0]+'.'+str2[1]+'.png'
                yield {'trimap_id': parts[0],
                       'trimap_file': os.path.join(folder, parts[1]),
                       'trimap_file_rel': parts[1]}

    def trimap_addrs(self, relative=False):
        """
        returns a hash from image_id to image address, which can be relative to the images_folder name or absolute.
        By default it generates absolute addresses.
        """
        all_infos = list(self.trimap())
        the_hash = {}

        info_key = 'trimap_file'
        if relative:
            info_key = 'trimap_file_rel'

        for info in all_infos:
            the_hash[int(info['trimap_id'])] = info[info_key]
...
class BerkeleyAnnotaionHelper(object):
    ...
    def get_train_berkeley_annotation(self, train_id, name):
        ...
        #return geometry_utils.Box(xmin, xmax, ymax, ymin)
        #return geometry_utils.Box(xmin, ymin, xmax, ymax)
        #return geometry_utils.Box(ymin, ymax, xmin, xmax)
        #return geometry_utils.Box(ymax, ymin, xmax, xmin)
